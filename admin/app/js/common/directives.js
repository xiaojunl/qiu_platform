...
//折扣系数验证(只能一位小数的非负数)
var rebate_REGEXP =/^(\d+.\d{1,1}|\d+)$/;
platDirective.directive('rebateformat', function() {
    return {
        require: 'ngModel',
        link: function(scope, elm, attrs, ctrl) {
            ctrl.$parsers.unshift(function(viewValue) {
                if (rebate_REGEXP.test(viewValue)) {
                    // it is valid
                    ctrl.$setValidity('rebateformat', true);
                    return viewValue;
                } else {
                    // it is invalid, return undefined (no model update)
                    ctrl.$setValidity('rebateformat', false);
                    return undefined;
                }
            });
        }
...
//匹配非负整数
var nonnegativeInteger_REGEXP = /^(0|[1-9]\d*)$/;
platDirective.directive('nonnegativeinteger', function() {
    return {
        require: 'ngModel',
        link: function(scope, elm, attrs, ctrl) {
            ctrl.$parsers.unshift(function(viewValue) {
                if (nonnegativeInteger_REGEXP.test(viewValue)) {
                    // it is valid
